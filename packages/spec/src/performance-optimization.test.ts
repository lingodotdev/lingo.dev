...
describe('LazyProviderConfigLoader', () => {
  ...
  test('should unload configurations to free memory', async () => {
    ...
    const mistralStats = stats.providerStats.find(s => s.provider === 'mistral');
  ...
  test('should emit events during loading', async () => {
    const events: string[] = [];
    loader.on('config:loading', (providerId) => events.push(`loading:${providerId}`));
    loader.on('config:loaded', (providerId) => events.push(`loaded:${providerId}`));
...
describe('OptimizedConfigParser', () => {
  ...
  test('should handle JSON string input', () => {
    const configString = JSON.stringify({ sourceLocale: 'en', targetLocales: ['es'] });
