...
import { LazyProviderConfigLoader, providerConfigLoader, ProviderId } from './lazy-provider-loader';
import { OptimizedConfigParser, configParser, LingoConfig } from './optimized-config-parser';
...
export class PerformanceOptimizedConfigSystem {
   ...
   */
  async loadConfig(source: string | object, cacheKey?: string): Promise<LingoConfig> {
    ...
    if (parseResult.errors && parseResult.errors.length > 0) {
      throw new Error(`Configuration errors: ${parseResult.errors.map(e => e.message).join(', ')}`);
    }
  ...
  async warmup(configs: string[]): Promise<void> {
    ...
    if (this.options.enableLazyLoading && this.options.preloadProviders.length > 0) {
  ...
  private async streamParseFile(filePath: string, cacheKey: string): Promise<LingoConfig> {
    const fs = await import('fs');
...
export type { LingoConfig, ProviderId, ProviderConfig } from './types';