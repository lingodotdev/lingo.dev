...
export class LCPAPI {
  ...
  private static _createAiModel(
    ...
    switch (providerId) {
      ...
      default: {
        throw new Error(
          `⚠️  Provider "${providerId}" for locale "${targetLocale}" is not supported. Only "groq", "google", "openrouter", "ollama", "aimlapi", "mistral", "openai", and "anthropic" providers are supported at the moment.`,
        );
      }
    }
  }
