  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (commentData.comment) {
      ...
      if (keyInfo && keyInfo.key) {
        setCommentAtPath(comments, keyInfo.path, keyInfo.key, commentData.comment);
      }
...
function extractCommentFromLine(line: string, lines: string[], lineIndex: number): { comment: string | null; lineIndex: number; endIndex: number; isInline: boolean } {
  const trimmed = line.trim();
  
  // Single-line comment (standalone)
  if (trimmed.startsWith('//')) {
    const comment = trimmed.replace(/^\/\/\s*/, '').trim();
    return { comment, lineIndex, endIndex: lineIndex, isInline: false };
  }
  ...
  if (singleInlineMatch && singleInlineMatch[1].includes(':')) {
    const comment = singleInlineMatch[2].trim();
    return { comment, lineIndex, endIndex: lineIndex, isInline: true };
  }
  
  // Handle block inline comments
  const blockInlineMatch = line.match(/^(.+?)\s*\/\*\s*(.*?)\s*\*\/.*$/);
  if (blockInlineMatch && blockInlineMatch[1].includes(':')) {
    const comment = blockInlineMatch[2].trim();
    return { comment, lineIndex, endIndex: lineIndex, isInline: true };
  }
  
  return { comment: null, lineIndex, endIndex: lineIndex, isInline: false };
}

function extractBlockComment(lines: string[], startIndex: number): { comment: string | null; lineIndex: number; endIndex: number } {
...
function findAssociatedKey(lines: string[], commentLineIndex: number, contextStack: Array<{ key: string; isArray: boolean }>): { key: string | null; path: string[] } {
...
function updateContext(contextStack: Array<{ key: string; isArray: boolean }>, line: string, parsedJson: any): void {
...
function setCommentAtPath(comments: Record<string, any>, path: string[], key: string, comment: string): void {
