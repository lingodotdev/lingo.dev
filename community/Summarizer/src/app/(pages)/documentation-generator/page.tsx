...
import { FileText, Wand2, LoaderCircle } from 'lucide-react';
import { DocumentationForm } from '@/components/documentation-form';
import { generateDocsAction } from '@/lib/actions';
...
export default function DocumentationGeneratorPage() {
  ...
  const handleFormAction = async (data: DocumentationFormData) => {
    setResult({ isPending: true });
    const response = await generateDocsAction(data);
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <p className="text-muted-foreground">
              Fill in the details about your Lingo tool, and our AI will generate
              professional, user-friendly documentation for you.
            </p>
          </div>
          <Card>
            <CardContent className="p-6">
              <DocumentationForm onFormAction={handleFormAction} isPending={result.isPending}/>
            </CardContent>
          </Card>
        </div>
        <div className="space-y-4">
                ...
                <CardContent className="p-6">
                    {result.isPending && (
                        <div className="flex h-full min-h-[400px] flex-col items-center justify-center gap-4 text-muted-foreground">
                            ...
                            <p className="text-sm">Generating your documentation now.</p>
                        </div>
                    )}
                    {result.error && (
                    ...
                    {result.documentation && (
                        <div className="prose prose-sm dark:prose-invert max-w-none">
                            <pre className="whitespace-pre-wrap bg-card font-sans">{result.documentation}</pre>
                        </div>
                    )}
                    {!result.isPending && !result.documentation && !result.error && (
                        <div className="flex h-full min-h-[400px] flex-col items-center justify-center gap-2 text-center text-muted-foreground">
                            <FileText className="h-12 w-12" />
                            <p className="font-headline text-lg">Your document will appear here</p>
