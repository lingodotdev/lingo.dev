...
> Intentionally empty frontmatter — no package release. This change only deletes an unused lockfile inside a `private` demo, so no published package is affected (matches the repo's `pnpm new:empty` convention for non-release PRs).
