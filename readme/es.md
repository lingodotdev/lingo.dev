...
| [**Compiler**](#lingodev-compiler) | Localización de React en tiempo de compilación sin wrappers i18n | Plugin `withLingo()`               |
...
| `pull-request`       | `false`                                        | Crear PR en lugar de hacer commit directamente |
...
La i18n tradicional es invasiva. Envuelves cada cadena en funciones `t()`, inventas claves de traducción (`home.hero.title.v2`), mantienes archivos JSON paralelos y ves cómo tus componentes se hinchan con código repetitivo de localización. Es tan tedioso que los equipos retrasan la internacionalización hasta que se convierte en una refactorización masiva.
...
**Desarrollo:** `npm run dev` (usa pseudotraductor, sin llamadas a API)
...
- Widget editor de traducciones integrado
...
- `cache-only`: Modo de producción usando traducciones pregeneradas desde CI (sin llamadas a API)
