...
Традиционный i18n — это морока. Нужно оборачивать каждую строку в функции `t()`, придумывать ключи переводов (`home.hero.title.v2`), поддерживать параллельные JSON-файлы и наблюдать, как компоненты раздуваются из-за локализационного шаблона. Всё настолько утомительно, что команды откладывают интернационализацию до тех пор, пока это не превращается в огромный рефакторинг.
