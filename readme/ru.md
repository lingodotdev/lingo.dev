...
1. Настройте роутинг по локали (например, `/en`, `/es`, `/pt-BR`)
...
4. Создаёт `i18n.lock` файл для отслеживания завершённых переводов (чтобы избежать повторной обработки)
...
Поле `provider` необязательно (по умолчанию используется Lingo.dev Engine). Для кастомных LLM-провайдеров:
...
2. Для PR workflow: Включите "Allow GitHub Actions to create and approve pull requests" в Settings > Actions > General
...
| `commit-message`     | `"feat: update translations via @LingoDotDev"` | Кастомное сообщение коммита       |
| `pull-request-title` | `"feat: update translations via @LingoDotDev"` | Кастомный заголовок PR            |
| `working-directory`  | `"."`                                          | Директория для выполнения         |
...
Традиционный i18n — это боль. Нужно оборачивать каждую строку в функции `t()`, придумывать ключи переводов (`home.hero.title.v2`), поддерживать параллельные JSON-файлы и наблюдать, как компоненты раздуваются от локализационного шаблона. Всё настолько муторно, что команды откладывают интернационализацию до момента, когда это превращается в огромный рефакторинг.
