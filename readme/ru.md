...
1. Настрой роутинг по локали (например, `/en`, `/es`, `/pt-BR`)
2. Добавь компоненты для переключения языка
3. Реализуй автоматическое определение локали
4. Сгенерируй нужные конфиги
...
1. Извлекает переводимый контент из указанных файлов
...
Команда `init` генерирует файл `i18n.json`. Настрой локали и бакеты:
...
Поле `provider` не обязательно (по умолчанию — Lingo.dev Engine). Для кастомных LLM-провайдеров:
...
Создай `.github/workflows/translate.yml`:
...
1. Добавь `LINGODOTDEV_API_KEY` в секреты репозитория (Settings > Secrets and variables > Actions)
2. Для PR-воркфлоу: Включи "Allow GitHub Actions to create and approve pull requests" в Settings > Actions > General
...
Традиционный i18n — это боль. Нужно оборачивать каждую строку в функции `t()`, придумывать ключи переводов (`home.hero.title.v2`), поддерживать параллельные JSON-файлы и наблюдать, как компоненты раздуваются от локализационного шаблона. Всё настолько муторно, что команды откладывают интернационализацию до тех пор, пока это не превращается в огромный рефакторинг.
