...
| ---------------------------------- | ----------------------------------------------------- | ---------------------------------- |
| [**MCP**](#lingodev-mcp)           | AI-помощник для настройки i18n в React-приложениях    | Prompt: `Set up i18n`              |
| [**CLI**](#lingodev-cli)           | Перевод файлов JSON, YAML, markdown, CSV, PO          | `npx lingo.dev@latest run`         |
| [**CI/CD**](#lingodev-cicd)        | Автоматизированный pipeline перевода в GitHub Actions | `uses: lingodotdev/lingo.dev@main` |
...
1. Настройте маршрутизацию по локали (например, `/en`, `/es`, `/pt-BR`)
2. Добавьте компоненты для переключения языка
3. Реализуйте автоматическое определение локали
4. Сгенерируйте необходимые конфигурационные файлы
...
2. Отправляет контент в LLM-провайдер для перевода
3. Записывает переведённый контент обратно в файловую систему
4. Создаёт `i18n.lock` файл для отслеживания завершённых переводов (чтобы не переводить повторно)
...
Поле `provider` не обязательно (по умолчанию используется Lingo.dev Engine). Для своих LLM-провайдеров:
...
2. Для PR-воркфлоу: Включите "Разрешить GitHub Actions создавать и одобрять pull requests" в Настройки > Actions > General
...
| -------------------- | ---------------------------------------------- | --------------------------------- |
| `api-key`            | (обязательно)                                  | Ключ API Lingo.dev                |
| `pull-request`       | `false`                                        | Создать PR вместо прямого коммита |
| `commit-message`     | `"feat: update translations via @LingoDotDev"` | Свой комментарий к коммиту        |
| `pull-request-title` | `"feat: update translations via @LingoDotDev"` | Свой заголовок PR                 |
| `working-directory`  | `"."`                                          | Каталог для выполнения            |
...
Традиционный i18n — это морока. Нужно оборачивать каждую строку в функции `t()`, придумывать ключи переводов (`home.hero.title.v2`), поддерживать параллельные JSON-файлы и наблюдать, как компоненты раздуваются от локализационного шаблона. Всё настолько утомительно, что команды откладывают интернационализацию до тех пор, пока это не превращается в огромный рефакторинг.
...
- Нулевая стоимость производительности на рантайме
- Без ключей переводов и JSON-файлов
- Без функций `t()` или обёрток-компонентов `<T>`
...
- `pseudotranslator`: Режим разработки с плейсхолдерными переводами (без затрат на API)
...
1. **Issues:** [Сообщить об ошибках или предложить фичи](https://github.com/lingodotdev/lingo.dev/issues)
2. **Pull Requests:** [Отправить изменения](https://github.com/lingodotdev/lingo.dev/pulls)
   - Для каждого PR нужен changeset: `pnpm new` (или `pnpm new:empty` для нерелизных изменений)
   - Перед отправкой убедитесь, что тесты проходят
3. **Разработка:** Это монорепозиторий pnpm + turborepo
