...
| [**CI/CD**](#lingodev-cicd)        | Автоматизация перевода в GitHub Actions            | `uses: lingodotdev/lingo.dev@main` |
...
Поле `provider` необязательно (по умолчанию используется Lingo.dev Engine). Для своих LLM-провайдеров:
...
Традиционный i18n — это вмешательство в код. Нужно оборачивать каждую строку в функции `t()`, придумывать ключи переводов (`home.hero.title.v2`), поддерживать параллельные JSON-файлы и наблюдать, как компоненты раздуваются из-за локализационного шаблона. Всё настолько муторно, что команды откладывают интернационализацию до тех пор, пока это не превращается в огромный рефакторинг.
