...
| [**CI/CD**](#lingodev-cicd)        | Автоматизированный перевод в GitHub Actions        | `uses: lingodotdev/lingo.dev@main` |
...
Традиционный i18n — это вмешательство в код. Нужно оборачивать каждую строку в функции `t()`, придумывать ключи переводов (`home.hero.title.v2`), поддерживать параллельные JSON-файлы и наблюдать, как компоненты раздуваются от шаблонного кода для локализации. Всё настолько муторно, что команды откладывают интернационализацию до тех пор, пока это не превращается в огромный рефакторинг.
...
- `pseudotranslator`: Режим разработки с плейсхолдерами переводов (без затрат на API)
