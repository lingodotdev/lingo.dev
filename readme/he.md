...
לאחר הגדרת שרת ה-MCP בסביבת הפיתוח שלך ([ראה מדריכי התחלה מהירה](https://lingo.dev/en/mcp)), בקש מהעוזר שלך:
...
i18n מסורתי הוא פולשני. אתם עוטפים כל מחרוזת בפונקציות `t()`, ממציאים מפתחות תרגום (`home.hero.title.v2`), מתחזקים קבצי JSON מקבילים, וצופים בקומפוננטות שלכם מתנפחות עם boilerplate של לוקליזציה. זה כל כך מייגע שצוותים דוחים בינלאומיות עד שהיא הופכת לרפקטור מסיבי.
...
1. **Issues:** [דיווח על באגים או בקשת פיצ'רים](https://github.com/lingodotdev/lingo.dev/issues)
2. **Pull Requests:** [שליחת שינויים](https://github.com/lingodotdev/lingo.dev/pulls)
   - כל PR דורש changeset: `pnpm new` (או `pnpm new:empty` עבור שינויים שאינם release)
