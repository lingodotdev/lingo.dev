...
Geleneksel i18n müdahaleci bir yaklaşımdır. Her metni `t()` fonksiyonlarına sarmalarsınız, çeviri anahtarları icat edersiniz (`home.hero.title.v2`), paralel JSON dosyaları yönetirsiniz ve bileşenlerinizin yerelleştirme standart kodlarıyla şişmesini izlersiniz. O kadar sıkıcıdır ki ekipler uluslararasılaştırmayı büyük bir yeniden yapılandırma haline gelene kadar erteler.
...
- `t()` fonksiyonları veya `<T>` sarmalayıcı bileşenler yok
