...
| 工具                               | 用例                                    | 快捷命令                           |
| ---------------------------------- | --------------------------------------- | ---------------------------------- |
| [**MCP**](#lingodev-mcp)           | React 应用的 AI 辅助 i18n 配置          | 提示：`Set up i18n`                |
...
传统的 i18n 方式具有侵入性。你需要将每个字符串包裹在 `t()` 函数中，手动创建翻译键（`home.hero.title.v2`），维护多份 JSON 文件，并且你的组件会因本地化样板代码而变得臃肿。这个过程非常繁琐，以至于团队往往会拖延国际化，直到最后变成一次大规模重构。
...
**开发环境：** `npm run dev`（使用伪翻译，无需 API 调用）
