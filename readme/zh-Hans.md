...
| [**CI/CD**](#lingodev-cicd)        | GitHub Actions 中的自动化翻译流程       | `uses: lingodotdev/lingo.dev@main` |
...
传统的 i18n 方式具有侵入性。你需要将每个字符串包裹在 `t()` 函数中，手动创建翻译键（`home.hero.title.v2`），维护多份 JSON 文件，并且你的组件会因为本地化样板代码而变得臃肿。这个过程非常繁琐，以至于团队往往会推迟国际化，直到不得不进行大规模重构。
...
1. 在 [`i18n.json`](./i18n.json) 中添加本地化代码，格式采用 [BCP-47 标准](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale)
2. 提交拉取请求

**BCP-47 语言区域格式：** `language[-Script][-REGION]`
