...
| సాధనం                              | ఉపయోగ సందర్భం                                     | త్వరిత కమాండ్                      |
| ---------------------------------- | ------------------------------------------------- | ---------------------------------- |
| [**MCP**](#lingodev-mcp)           | React యాప్‌ల కోసం AI-సహాయిత i18n సెటప్            | ప్రాంప్ట్: `Set up i18n`           |
...
| [**Compiler**](#lingodev-compiler) | i18n రాపర్స్ లేకుండా బిల్డ్-టైమ్ React లోకలైజేషన్ | `withLingo()` ప్లగిన్              |
...
1. లొకేల్-ఆధారిత రూటింగ్‌ను కాన్ఫిగర్ చేయండి (ఉదా., `/en`, `/es`, `/pt-BR`)
2. భాషా మార్పిడి కాంపోనెంట్‌లను సెటప్ చేయండి
3. ఆటోమేటిక్ లొకేల్ డిటెక్షన్‌ను అమలు చేయండి
4. అవసరమైన కాన్ఫిగరేషన్ ఫైల్స్‌ను జనరేట్ చేయండి
...
3. అనువదించిన కంటెంట్‌ను ఫైల్‌సిస్టమ్‌కు తిరిగి వ్రాస్తుంది
4. పూర్తయిన అనువాదాలను ట్రాక్ చేయడానికి `i18n.lock` ఫైల్‌ను సృష్టిస్తుంది (రిడండెంట్ ప్రాసెసింగ్‌ను నివారిస్తుంది)
...
- జీరో రన్‌టైమ్ పెర్ఫార్మెన్స్ ఖర్చు
- అనువాద కీలు లేదా JSON ఫైల్‌లు లేవు
...
- `cache-only`: CIనుండి ముందుగా జనరేట్ చేసిన అనువాదాలను ఉపయోగించి ప్రొడక్షన్ మోడ్ (API కాల్స్ లేవు)
...
2. **పుల్ రిక్వెస్ట్‌లు:** [మార్పులను సబ్మిట్ చేయండి](https://github.com/lingodotdev/lingo.dev/pulls)
   - ప్రతి PRకు చేంజ్‌సెట్ అవసరం: `pnpm new` (లేదా నాన్-రిలీజ్ మార్పుల కోసం `pnpm new:empty`)
   - సబ్మిట్ చేయడానికి ముందు టెస్ట్‌లు పాస్ అవుతున్నాయని నిర్ధారించుకోండి
...
1. [BCP-47 ఫార్మాట్](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale)ను ఉపయోగించి [`i18n.json`](./i18n.json)కు లొకేల్ కోడ్‌ను జోడించండి
2. పుల్ రిక్వెస్ట్‌ను సబ్మిట్ చేయండి

**BCP-47 లొకేల్ ఫార్మాట్:** `language[-Script][-REGION]`

- `language`: ISO 639-1/2/3 (లోయర్‌కేస్): `en`, `zh`, `bho`
- `Script`: ISO 15924 (టైటిల్ కేస్): `Hans`, `Hant`, `Latn`
- `REGION`: ISO 3166-1 alpha-2 (అప్పర్‌కేస్): `US`, `CN`, `IN`
