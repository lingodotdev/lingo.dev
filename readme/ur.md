...
3. خودکار لوکیل کی شناخت کو نافذ کریں
4. ضروری کنفیگریشن فائلیں تیار کریں
...
2. ترجمے کے لیے مواد کو LLM فراہم کنندہ کو بھیجتا ہے
...
`init` کمانڈ ایک `i18n.json` فائل تیار کرتی ہے۔ لوکیلز اور بکٹس کو کنفیگر کریں:
...
1. ریپوزٹری سیکرٹس میں `LINGODOTDEV_API_KEY` شامل کریں (Settings > Secrets and variables > Actions)
...
| `pull-request`       | `false`                                        | براہ راست کمٹ کرنے کی بجائے PR بنائیں |
| `commit-message`     | `"feat: update translations via @LingoDotDev"` | حسب ضرورت کمٹ پیغام                   |
...
روایتی i18n مداخلت آمیز ہے۔ آپ ہر string کو `t()` functions میں wrap کرتے ہیں، translation keys ایجاد کرتے ہیں (`home.hero.title.v2`)، متوازی JSON files کو maintain کرتے ہیں، اور اپنے components کو localization boilerplate سے بھرا ہوا دیکھتے ہیں۔ یہ اتنا تکلیف دہ ہے کہ teams internationalization کو اس وقت تک ملتوی کر دیتی ہیں جب تک کہ یہ ایک بڑے refactor میں تبدیل نہ ہو جائے۔
...
- Runtime performance کی کوئی قیمت نہیں
...
1. **Issues:** [Bugs رپورٹ کریں یا features کی درخواست کریں](https://github.com/lingodotdev/lingo.dev/issues)
