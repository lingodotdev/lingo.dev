...
| ---------------------------------- | ------------------------------------------------------- | ---------------------------------- |
| [**MCP**](#lingodev-mcp)           | راه‌اندازی i18n با کمک هوش مصنوعی برای برنامه‌های React | Prompt: `Set up i18n`              |
| [**CLI**](#lingodev-cli)           | ترجمه فایل‌های JSON، YAML، markdown، CSV، PO            | `npx lingo.dev@latest run`         |
| [**CI/CD**](#lingodev-cicd)        | خط لوله ترجمه خودکار در GitHub Actions                  | `uses: lingodotdev/lingo.dev@main` |
...
i18n سنتی مزاحم است. شما هر رشته را در توابع `t()` می‌پیچید، کلیدهای ترجمه را اختراع می‌کنید (`home.hero.title.v2`)، فایل‌های JSON موازی را نگهداری می‌کنید و شاهد متورم شدن کامپوننت‌هایتان با کدهای اضافی محلی‌سازی هستید. این کار آنقدر خسته‌کننده است که تیم‌ها بین‌المللی‌سازی را به تعویق می‌اندازند تا زمانی که به یک بازسازی عظیم تبدیل شود.
...
دایرکتوری `.lingo/` را در کنترل نسخه commit کنید.
...
- بازنویسی دستی از طریق ویژگی `data-lingo-override`
- ویجت ویرایشگر ترجمه داخلی

**حالت‌های build:**

- `pseudotranslator`: حالت توسعه با ترجمه‌های جایگزین (بدون هزینه API)
