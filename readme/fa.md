...
i18n سنتی مزاحم است. شما هر رشته را در توابع `t()` می‌پیچید، کلیدهای ترجمه را ابداع می‌کنید (`home.hero.title.v2`)، فایل‌های JSON موازی را نگهداری می‌کنید و شاهد باد کردن کامپوننت‌هایتان با کدهای اضافی محلی‌سازی هستید. این کار آنقدر خسته‌کننده است که تیم‌ها بین‌المللی‌سازی را تا زمانی که به یک بازسازی عظیم تبدیل شود، به تعویق می‌اندازند.
