...
| औजार                               | उपयोग के मामला                                       | त्वरित कमांड                       |
| ---------------------------------- | ---------------------------------------------------- | ---------------------------------- |
...
| [**SDK**](#lingodev-sdk)           | गतिशील सामग्री खातिर रनटाइम अनुवाद                   | `npm install lingo.dev`            |
...
1. लोकेल-आधारित रूटिंग कॉन्फ़िगर करीं (जइसे कि, `/en`, `/es`, `/pt-BR`)
2. भाषा स्विचिंग कंपोनेंट सेट अप करीं
3. स्वचालित लोकेल पहचान लागू करीं
4. जरूरी कॉन्फ़िगरेशन फाइल सभ जेनरेट करीं
...
2. अनुवाद खातिर LLM प्रदाता के सामग्री भेजेला
3. अनुवादित सामग्री के फाइल सिस्टम में वापस लिखेला
4. पूरा भइल अनुवाद सभ के ट्रैक करे खातिर `i18n.lock` फाइल बनावेला (अनावश्यक प्रोसेसिंग से बचावेला)

**कॉन्फ़िगरेशन:**

`init` कमांड एगो `i18n.json` फाइल जेनरेट करेला। लोकेल आ बकेट सभ कॉन्फ़िगर करीं:
...
`provider` फील्ड वैकल्पिक बा (डिफ़ॉल्ट Lingo.dev Engine बा)। कस्टम LLM प्रदाता खातिर:
...
1. रिपॉजिटरी सीक्रेट्स में `LINGODOTDEV_API_KEY` जोड़ीं (Settings > Secrets and variables > Actions)
...
| -------------------- | ---------------------------------------------- | ------------------------------ |
| `api-key`            | (जरूरी)                                        | Lingo.dev API कुंजी            |
...
| `working-directory`  | `"."`                                          | चलावे खातिर डायरेक्टरी         |
...
पारंपरिक i18n घुसपैठिया बा। रउआ हर स्ट्रिंग के `t()` फंक्शन में लपेट देनी, अनुवाद कुंजी (`home.hero.title.v2`) बनावे के पड़ेला, समानांतर JSON फाइल के रखरखाव करे के पड़ेला, आ आपन कंपोनेंट के लोकलाइजेशन बॉयलरप्लेट से फूलल देखे के पड़ेला। ई एतना थकाऊ बा कि टीम सभ अंतर्राष्ट्रीयकरण के तब तक टाल देवेला जब तक ई एगो बहुत बड़ रिफैक्टर ना बन जाला।
...
**डेवलपमेंट:** `npm run dev` (स्यूडोट्रांसलेटर के इस्तेमाल करेला, कवनो API कॉल नइखे)
...
- रनटाइम परफॉर्मेंस के कवनो लागत नइखे
- कवनो अनुवाद कुंजी भा JSON फाइल नइखे
- कवनो `t()` फंक्शन भा `<T>` रैपर कंपोनेंट नइखे
...
- `real`: LLM के इस्तेमाल से वास्तविक अनुवाद जेनरेट करीं
- `cache-only`: CI से पहिले से जेनरेट कइल अनुवाद के इस्तेमाल करत प्रोडक्शन मोड (कवनो API कॉल नइखे)
...
1. **इश्यू:** [बग रिपोर्ट करीं भा फीचर रिक्वेस्ट करीं](https://github.com/lingodotdev/lingo.dev/issues)
2. **पुल रिक्वेस्ट:** [बदलाव सबमिट करीं](https://github.com/lingodotdev/lingo.dev/pulls)
   - हर PR खातिर एगो चेंजसेट जरूरी बा: `pnpm new` (भा नॉन-रिलीज बदलाव खातिर `pnpm new:empty`)
   - सबमिट करे से पहिले सुनिश्चित करीं कि टेस्ट पास हो जाला
3. **डेवलपमेंट:** ई एगो pnpm + turborepo मोनोरेपो बा
...
1. [BCP-47 फॉर्मेट](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl/Locale) के इस्तेमाल करत [`i18n.json`](./i18n.json) में लोकेल कोड जोड़ीं
